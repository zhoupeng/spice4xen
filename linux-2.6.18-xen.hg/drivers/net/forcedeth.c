...
#define DEV_HAS_CORRECT_MACADDR    0x02000  /* device supports correct mac address */
	...
	for (i = 0; i < np->rx_ring_size; i++) {
		...
		if (np->rx_skb[i].skb) {
			...
			pci_unmap_single(np->pci_dev, np->rx_skb[i].dma,
					(skb_end_pointer(np->rx_skb[i].skb) - np->rx_skb[i].skb->data),
					PCI_DMA_FROMDEVICE);
#else
			pci_unmap_single(np->pci_dev, np->rx_skb[i].dma,
	...
	while((np->get_rx.orig != np->put_rx.orig) &&
		...
		if (np->desc_ver == DESC_VER_1) {
			...
			if (likely(Flags & NV_RX2_DESCRIPTORVALID)) {
				...
				if (((Flags & NV_RX2_CHECKSUMMASK) == NV_RX2_CHECKSUM_IP_TCP) || ((Flags & NV_RX2_CHECKSUMMASK) == NV_RX2_CHECKSUM_IP_UDP)) 
					/*ip and tcp or udp */
	...
	while((np->get_rx.ex != np->put_rx.ex) &&
		...
		if (likely(Flags & NV_RX2_DESCRIPTORVALID)) {
			...
			if (likely(np->rx_csum)) {
				if (likely(((Flags & NV_RX2_CHECKSUMMASK) == NV_RX2_CHECKSUM_IP_TCP) || ((Flags & NV_RX2_CHECKSUMMASK) == NV_RX2_CHECKSUM_IP_UDP))) 
					/*ip and tcp or udp */
					skb->ip_summed = CHECKSUM_UNNECESSARY;
			}
...
static int nv_request_irq(struct net_device *dev, int intr_test)
{
	...
	if (np->msi_flags & NV_MSI_X_CAPABLE) {
		...
		if ((ret = pci_enable_msix(np->pci_dev, np->msi_x_entry, (np->msi_flags & NV_MSI_X_VECTORS_MASK))) == 0) {
			...
			if (optimization_mode == NV_OPTIMIZATION_MODE_THROUGHPUT && !intr_test) {
				...
				if (request_irq(np->msi_x_entry[NV_MSI_X_VECTOR_RX].vector, &nv_nic_irq_rx, IRQ_FLAG, dev->name, dev) != 0) {
				...
				if (request_irq(np->msi_x_entry[NV_MSI_X_VECTOR_TX].vector, &nv_nic_irq_tx, IRQ_FLAG, dev->name, dev) != 0) {
				...
				if (request_irq(np->msi_x_entry[NV_MSI_X_VECTOR_OTHER].vector, &nv_nic_irq_other, IRQ_FLAG, dev->name, dev) != 0) {
				...
				if ((!intr_test && np->desc_ver == DESC_VER_3 &&
							request_irq(np->msi_x_entry[NV_MSI_X_VECTOR_ALL].vector, &nv_nic_irq_optimized, IRQ_FLAG, dev->name, dev) != 0) ||
						(!intr_test && np->desc_ver != DESC_VER_3 &&
						 request_irq(np->msi_x_entry[NV_MSI_X_VECTOR_ALL].vector, &nv_nic_irq, IRQ_FLAG, dev->name, dev) != 0) ||
						(intr_test &&
						 request_irq(np->msi_x_entry[NV_MSI_X_VECTOR_ALL].vector, &nv_nic_irq_test, IRQ_FLAG, dev->name, dev) != 0)) {
	...
	if (ret != 0 && np->msi_flags & NV_MSI_CAPABLE) {
		if ((ret = pci_enable_msi(np->pci_dev)) == 0) {
			...
			if ((!intr_test && np->desc_ver == DESC_VER_3 &&
						request_irq(np->pci_dev->irq, &nv_nic_irq_optimized, IRQ_FLAG, dev->name, dev) != 0) ||
					(!intr_test && np->desc_ver != DESC_VER_3 &&
					 request_irq(np->pci_dev->irq, &nv_nic_irq, IRQ_FLAG, dev->name, dev) != 0) ||
					(intr_test && request_irq(np->pci_dev->irq, &nv_nic_irq_test, IRQ_FLAG, dev->name, dev) != 0)) {
				printk(KERN_INFO "forcedeth: request_irq failed %d\n", ret);
...
static int __devinit nv_probe(struct pci_dev *pci_dev, const struct pci_device_id *id)
{
	...
	if (!phyinitialized) {		
		...
		np->autoneg = (reg & BMCR_ANENABLE ? AUTONEG_ENABLE:AUTONEG_DISABLE);
		if(np->autoneg == AUTONEG_DISABLE){
			reg = mii_rw(dev, np->phyaddr, MII_ADVERTISE, MII_READ);
			np->fixed_mode = reg;
		}
...
static struct pci_device_id pci_tbl[] = {
	...
	{	/* MCP65 Ethernet Controller */
		...
		.driver_data = DEV_NEED_TIMERIRQ|DEV_NEED_LINKTIMER|DEV_HAS_LARGEDESC|DEV_HAS_CHECKSUM|DEV_HAS_HIGH_DMA|DEV_HAS_POWER_CNTRL|DEV_HAS_MSI|DEV_HAS_PAUSEFRAME_TX_V1|DEV_HAS_STATISTICS_V2|DEV_HAS_TEST_EXTENDED|DEV_HAS_MGMT_UNIT|DEV_HAS_CORRECT_MACADDR,
	},
	{	/* MCP65 Ethernet Controller */
	...
	{	/* MCP77 Ethernet Controller */
		...
		.driver_data = DEV_NEED_TIMERIRQ|DEV_NEED_LINKTIMER|DEV_HAS_CHECKSUM|DEV_HAS_HIGH_DMA|DEV_HAS_MSI|DEV_HAS_POWER_CNTRL|DEV_HAS_PAUSEFRAME_TX_V2|DEV_HAS_STATISTICS_V2|DEV_HAS_TEST_EXTENDED|DEV_HAS_MGMT_UNIT|DEV_HAS_CORRECT_MACADDR,
	},
	{	/* MCP77 Ethernet Controller */
	...
	{	/* MCP79 Ethernet Controller */
		...
		.driver_data = DEV_NEED_TIMERIRQ|DEV_NEED_LINKTIMER|DEV_HAS_CHECKSUM|DEV_HAS_HIGH_DMA|DEV_HAS_MSI|DEV_HAS_POWER_CNTRL|DEV_HAS_PAUSEFRAME_TX_V3|DEV_HAS_STATISTICS_V2|DEV_HAS_TEST_EXTENDED|DEV_HAS_MGMT_UNIT|DEV_HAS_CORRECT_MACADDR,
	},
	{	/* MCP79 Ethernet Controller */
